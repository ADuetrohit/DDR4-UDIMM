...
| Date | Change |
|---|---|
| 2026-09-23 | **[docs/ROUTING_GUIDE.pdf](docs/ROUTING_GUIDE.pdf)** added, generated by `tools/gen_routing_guide.py` from the PcbDoc: layer stack, layer-assignment rules, a section per layer, all eight byte lanes by designator with DRAM ball and target, address/clock/ZQ/ALERT/SPD, the 18 differential pairs, special cases and what is left to route |
| 2026-09-23 | `tools/tune_byte.py` added: plans meander teeth on both halves of every line in a byte until each reaches the byte's longest line, checking clearance against tracks, arcs, vias and pads, and writes one Altium script. Byte 0: 9 of 11 lines reach target (`hardware/scripts/Byte0_Tune.pas`) |
